...
import {
  ...
  labelValues,
  patterns,
} from '../labels/namespace-gateway';
import { buildDsQueryHandler, extractPathSegment } from './_util';

/**
 * Scenario: user has drilled into `{namespace="gateway"}` from the namespace
 * primary-label tab. Used by Part 1 of the `tabs - namespace` block in
 * `tests/exploreServices.spec.ts`.
 */
export async function loadBreakdownNamespaceGateway(page: Page) {
