...
/* eslint-disable sort/string-unions */
/* eslint-disable sort/type-properties */
import { api } from './baseAPI';
export const addTagTypes = ['API Discovery', 'Query'] as const;
  ...
  .injectEndpoints({
    endpoints: (build) => ({
      getApiResources: build.query<GetApiResourcesApiResponse, GetApiResourcesApiArg>({
        query: () => ({ url: `/apis/queries.grafana.app/v1beta1/` }),
        providesTags: ['API Discovery'],
      }),
      listQuery: build.query<ListQueryApiResponse, ListQueryApiArg>({
        query: (queryArg) => ({
          url: `/queries`,
          params: {
      ...
      deletecollectionQuery: build.mutation<DeletecollectionQueryApiResponse, DeletecollectionQueryApiArg>({
        query: (queryArg) => ({
          url: `/queries`,
          method: 'DELETE',
          params: {
      ...
      replaceQuery: build.mutation<ReplaceQueryApiResponse, ReplaceQueryApiArg>({
        query: (queryArg) => ({
          url: `/queries/${queryArg.name}`,
          method: 'PUT',
          body: queryArg.query,
      ...
      updateQuery: build.mutation<UpdateQueryApiResponse, UpdateQueryApiArg>({
        query: (queryArg) => ({
          url: `/queries/${queryArg.name}`,
          method: 'PATCH',
          body: queryArg.patch,
      ...
      getQueryRender: build.query<GetQueryRenderApiResponse, GetQueryRenderApiArg>({
        query: (queryArg) => ({ url: `/queries/${queryArg.name}/render` }),
        providesTags: ['Query'],
      }),
      postApisQueriesGrafanaAppV1Beta1Render: build.mutation<
        PostApisQueriesGrafanaAppV1Beta1RenderApiResponse,
        PostApisQueriesGrafanaAppV1Beta1RenderApiArg
      >({
        query: (queryArg) => ({
          url: `/apis/queries.grafana.app/v1beta1/render`,
          method: 'POST',
          body: queryArg.body,
          params: {
...
export type ListQueryApiResponse = /** status 200 OK */ QueryList;
export type ListQueryApiArg = {
...
export type CreateQueryApiResponse =
  /** status 200 OK */
  Query | /** status 201 Created */ Query | /** status 202 Accepted */ Query;
...
export type DeletecollectionQueryApiResponse = /** status 200 OK */ Status;
export type DeletecollectionQueryApiArg = {
  /** If 'true', then the output is pretty printed. Defaults to 'false' unless the user-agent indicates a browser or command-line HTTP tool (curl and wget). */
  pretty?: string;
  /** The continue option should be set when retrieving more results from the server. Since this value is server defined, clients may only use the continue value from a previous query result with identical query parameters (except for the value of continue) and the server may reject a continue value it does not recognize. If the specified continue value is no longer valid whether due to expiration (generally five to fifteen minutes) or a configuration change on the server, the server will respond with a 410 ResourceExpired error together with a continue token. If the client needs a consistent list, it must restart their list without the continue field. Otherwise, the client may send another list request with the token received with the 410 error, the server will respond with a list starting from the next key, but from the latest snapshot, which is inconsistent from the previous list results - objects that are created, modified, or deleted after the first list request will be included in the response, as long as their keys are after the "next key".
...
export type GetQueryApiResponse = /** status 200 OK */ Query;
...
export type ReplaceQueryApiResponse = /** status 200 OK */ Query | /** status 201 Created */ Query;
export type ReplaceQueryApiArg = {
  /** name of the Query */
  name: string;
  /** If 'true', then the output is pretty printed. Defaults to 'false' unless the user-agent indicates a browser or command-line HTTP tool (curl and wget). */
  pretty?: string;
...
export type DeleteQueryApiResponse = /** status 200 OK */ Status | /** status 202 Accepted */ Status;
...
export type UpdateQueryApiResponse = /** status 200 OK */ Query | /** status 201 Created */ Query;
export type UpdateQueryApiArg = {
  /** name of the Query */
  name: string;
  /** If 'true', then the output is pretty printed. Defaults to 'false' unless the user-agent indicates a browser or command-line HTTP tool (curl and wget). */
  pretty?: string;
...
export type GetQueryRenderApiResponse = /** status 200 OK */ string;
export type GetQueryRenderApiArg = {
  /** name of the RenderedQuery */
  name: string;
};
export type PostApisQueriesGrafanaAppV1Beta1RenderApiResponse = /** status 200 OK */ {
...
export type PostApisQueriesGrafanaAppV1Beta1RenderApiArg = {
  /** Each variable is prefixed with var-{variable}={value} */
  variables?: {
    [key: string]: string[];
  };
  body: any;
...
export type Target = {
  ...
  variables: {
    [key: string]: VariableReplacement[];
  };
};
...
export type QuerySpec = {
  ...
  description?: string;
  /** Whether the query is locked and cannot be edited Note: This is purely for UI display purposes and not for security */
  isLocked?: boolean;
