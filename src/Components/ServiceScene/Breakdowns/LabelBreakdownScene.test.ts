...
describe('buildLabelsQuery', () => {
  test('should build no-parser query with no filters', () => {
    ...
    expect(result).toMatchObject({
      expr: `sum(count_over_time({\${filters} ,cluster != ""}  \${levels} \${metadata} \${patterns} \${lineFilters}  \${fields} \${lineFormat} [$__auto])) by (${VAR_LABEL_GROUP_BY_EXPR})`,
    });
  });
  ...
  test('should build logfmt-parser query with structured metadata filters', () => {
    ...
    const result = buildLabelsQuery({} as SceneObject, VAR_LABEL_GROUP_BY_EXPR, 'cluster');
    expect(result).toMatchObject({
      expr: `sum(count_over_time({\${filters} ,cluster != ""}  \${levels} \${metadata} \${patterns} \${lineFilters} | logfmt  \${fields} \${lineFormat} [$__auto])) by (${VAR_LABEL_GROUP_BY_EXPR})`,
    });
  });
  test('should build json-parser query with structured metadata filters', () => {
    ...
    const result = buildLabelsQuery({} as SceneObject, VAR_LABEL_GROUP_BY_EXPR, 'cluster');
    expect(result).toMatchObject({
      expr: `sum(count_over_time({\${filters} ,cluster != ""}  \${levels} \${metadata} \${patterns} \${lineFilters} | json \${jsonFields} | drop __error__, __error_details__  \${fields} \${lineFormat} [$__auto])) by (${VAR_LABEL_GROUP_BY_EXPR})`,
    });
  });
  test('should build mixed-parser query with structured metadata filters', () => {
    ...
    const result = buildLabelsQuery({} as SceneObject, VAR_LABEL_GROUP_BY_EXPR, 'cluster');
    expect(result).toMatchObject({
      expr: `sum(count_over_time({\${filters} ,cluster != ""}  \${levels} \${metadata} \${patterns} \${lineFilters} | json \${jsonFields} | logfmt | drop __error__, __error_details__  \${fields} \${lineFormat} [$__auto])) by (${VAR_LABEL_GROUP_BY_EXPR})`,
    });
  });
});
